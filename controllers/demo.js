...
const createOrder = asyncWrapper(async (req, res) => {
    const { paymentMethod, orderFrequency, note, purchaseOrderNumber, items, isLocalPartner, localPartnerId } = req.body;
    ...
    // ─── Local Partner ON — wholeSalePrice ────
    if (isLocalPartner === true || isLocalPartner === "true") {
        ...
        if (!partner) return errorResponse({ res, message: "Local partner not found", status: 404 });
